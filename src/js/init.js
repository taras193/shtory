...
if (isMobile == true) {
	...
	if(!$('body').hasClass('pre-loaded')){$('body').addClass('pre-loaded');}
	$('<style>body>header{height:800px!important}</style>').appendTo('head');
	console.log('mobile-inject,body.pre-loaded,body.loaded');
}
...
function initalize(){
	...
	// hide-loader
	if(!$('body').hasClass('pre-loaded')){$('body').addClass('pre-loaded');}
	console.log('body.pre-loaded');	

	// lazy-load
	$('img.lazy').each(function(index, el) { $(this).attr('src',$(this).data('original'))});
